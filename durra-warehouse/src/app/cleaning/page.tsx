...
export default function Page() {
  ...
  useEffect(() => {
    if (!user) return;
    getDocs(query(collection(db, "warehouse"), where("status", "==", "cleaning")))
    ...
    <div className="page-wrap">
      <div className="page-header">
        ...
        <div style={{ marginTop: 16 }}>
          <div className="page-header-title">التنظيف والصيانة</div>
          ...
          {filtered.length === 0 ? (
            ...
            <div key={item.id} className="item-card">
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                <div style={{ width: 10, height: 10, borderRadius: "50%", background: "#D97706", flexShrink: 0, marginTop: 4 }} />
              ...
              <button onClick={() => updateStatus(item.id)} disabled={updating === item.id} className="btn-green" style={{ opacity: updating === item.id ? 0.6 : 1 }}>
                {updating === item.id ? "جاري التحديث..." : "تم التنظيف ✓"}
              </button>
            </div>
          ))}
        </div>
