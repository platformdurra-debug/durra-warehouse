...
export default function WarehousePage() {
  ...
  useEffect(() => {
    if (!user) return;
    getDocs(query(collection(db, "warehouse"), where("status", "==", "available")))
      .then(snap => { setItems(snap.docs.map(d => ({ id: d.id, ...d.data() }))); setFetching(false); });
  }, [user]);

  const updateStatus = async (id: string) => {
    await updateDoc(doc(db, "warehouse", id), { status: "transit", updatedAt: new Date() });
  ...
  return (
    <div style={{ background: "#0A1628", minHeight: "100vh", paddingBottom: 90, fontFamily: "Tajawal, sans-serif", direction: "rtl" }}>
      <div style={{ padding: "56px 20px 16px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 20, fontWeight: 700, color: "#fff", textAlign: "center", marginBottom: 4 }}>{📤} إرسال الطلبات</div>
      ...
      <div style={{ padding: "12px 20px", display: "flex", flexDirection: "column", gap: 10 }}>
        {filtered.length === 0 ? (
          <div style={{ textAlign: "center", padding: "60px 20px" }}>
            <div style={{ fontSize: 48, marginBottom: 12 }}>{`📤 إرسال الطلبات`}</div>
